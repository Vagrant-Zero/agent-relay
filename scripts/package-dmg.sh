...
set -euo pipefail
cd "${0:A:h:h}"
app_dir="$PWD/dist/Agent Meter Preview.app"
[[ -d "$app_dir" ]] || { print -u2 -- '请先运行 ./scripts/build.sh'; exit 1; }
version="$(/usr/libexec/PlistBuddy -c 'Print CFBundleShortVersionString' "$app_dir/Contents/Info.plist")"
[[ "$version" =~ '^[0-9]+\.[0-9]+\.[0-9]+$' ]] || exit 1
for binary in AgentMeterPreview agent-meter agent-meter-bridge; do
...
mkdir "$stage/content"
ditto "$app_dir" "$stage/content/Agent Meter Preview.app"
ln -s /Applications "$stage/content/Applications"
cat > "$stage/content/安装说明.txt" <<'TXT'
Agent Meter Preview · macOS 26+ · Apple Silicon

将 Agent Meter Preview.app 拖到 Applications 后打开。
...
TXT
filename="Agent-Meter-$version-macos-arm64.dmg"
hdiutil create -volname 'Agent Meter' -srcfolder "$stage/content" -format UDZO "$stage/$filename"
