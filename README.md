...
- **查看额度**：显示各周期剩余额度、重置时间和可用重置卡数量；有可用且未过期的重置卡时，显示最近到期时间。接口仅返回部分详情时标为“已知最近到期”，未提供时间时不推测。
...
升级时退出 Agent Relay，再用新版覆盖应用即可。账号与历史不存储在应用包内，不会因覆盖应用而删除。从 v0.3.0 起，可在「设置 → 检查更新」下载并安装新版；默认每天检查一次，安装前由你确认。旧版用户需要先手动安装一次 v0.3.0 或更高版本。
...
要在菜单栏直接查看额度，勾选 **菜单栏显示剩余额度**。显示内容跟随当前 CLI 默认账号；`*` 表示缓存或查询异常，可打开菜单刷新。取消勾选后只显示图标，选择会自动保存。在 **额度周期** 中可固定选择 5 小时、每周或同时显示；查询未返回所选周期时显示 `—`，不会自动改成其他周期。
...
菜单栏 → **设置…**，或点击管理窗口右上角的齿轮。设置页整合背景透明度、菜单栏额度显示及周期、当前版本、检查更新和自动检查开关。

更新采用 Sparkle 2，通过 HTTPS 获取版本信息并验证 Ed25519 签名；更新器只在菜单栏进程运行，安装前等待账号操作完成并关闭管理窗口，账号、凭据和会话数据保留。安装包继续通过 GitHub Releases 分发。
